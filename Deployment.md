...
* **公网出口 / 弹性 EIP (EIP)**
  * **角色**: 平台对外暴露的唯一互联网入口。
  * **规格**: 静态公网 IP，带宽根据峰值预估（如 50-100Mbps 独享或按量计费）。
* **高防 IP 洗流中心 (Anti_DDoS)**
  * **角色**: 抵御大流量 DDoS 及泛洪攻击。
  * **等保要求**: 将异常流量清洗后，仅放行纯净流量进入内部 VPC。
* **WAF / 云防火墙 (WAF)**
  * **角色**: 七层应用防火墙。
  * **等保要求**: 拦截 OWASP Top 10 漏洞利用、防恶意爬虫及 CC 攻击，开启 HTTPS 强加密卸载。
* **云负载均衡 SLB (SLB)**
  * **角色**: 公网流量高可用分发。
  * **规格建议**: 独享型实例，支持跨多可用区部署。
* **内网反向代理 (Nginx1 / Nginx2)**
  * **角色**: 负责动静分离、内网流量路由与微服务入口分发。
  * **规格建议**: `4核 CPU / 8GB 内存 / 50GB 高性能系统盘`。两台做 Keepalived/主备高可用。
...
### 3.4 核心数据存储区节点
* **MySQL 主库 (MySQL_Master)**
  * **角色**: 承载全站核心写入与结构化事务处理。
  * **等保要求**: 严禁绑定外网 IP，必须开启 KMS 落盘透明加密。
  * **规格建议**: `16核 CPU / 64GB 内存 / 500GB+ SSD 极速云盘` (强调高 IOPS)。
* **MySQL 从库 (MySQL_Slave1 / MySQL_Slave2)**
  * **角色**: 承载只读流量及数据同步容灾，部署在不同可用区。
  * **规格建议**: `8核 CPU / 32GB 内存 / 500GB+ SSD`（单节点规格可略低于主库，根据读写比例横向扩展）。

### 3.5 缓存与消息中间件节点
* **Redis 集群 (Redis)**
  * **角色**: 分布式缓存集群，用于热点数据查询和分布式锁。
  * **等保要求**: 配置复杂的强密码访问鉴权，关闭危险指令。
  * **规格建议**: `8核 CPU / 32GB 内存 / 100GB 系统盘`（部署为至少 3 主 3 从的 Cluster）。
* **Kafka 消息队列 (Kafka)**
  * **角色**: 处理高吞吐量异步消息及削峰填谷。
  * **规格建议**: `8核 CPU / 32GB 内存 / 100GB系统盘 + 500GB 数据盘`（至少 3 节点组成的高吞吐集群）。

### 3.6 安全管理中心节点 (带外运维区)
* **堡垒机 (Bastion)**
  * **角色**: 企业内部唯一的特权运维跳板机。
  * **等保要求**: 强制开启多因素身份认证（MFA），录屏并审计所有运维指令。
  * **规格建议**: `2核 CPU / 4GB 内存`。
* **云安全中心 / 态势感知 (SOC)**
  * **角色**: 集中汇聚边界防火墙与主机终端 Agent 的安全告警，自动化响应威胁。
  * **规格建议**: SaaS 化服务 或 `4核 CPU / 16GB 内存` 的私有化探针引擎。
* **日志审计中心 (LogCenter)**
  * **角色**: 全量收集各应用、系统、数据库的访问与操作日志。
  * **等保要求**: 日志数据必须不可篡改，且保存周期严格 **大于 180 天**。
  * **规格建议**: `16核 CPU / 32GB 内存 / T 级别大容量低频存储盘`（可采用 ELK Stack 或云原生日志服务 SLS）。
