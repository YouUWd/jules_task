...
|  +--------------------+  +--------------------+  +--------------------+           |
|  |     PC Web端       |  |     移动端 H5/APP  |  |      微信小程序    |           |
|  | (Vue 3/React 18)   |  | (Taro/UniApp)      |  | (微信原生/UniApp)  |           |
|  +--------------------+  +--------------------+  +--------------------+           |
+-----------------------------------------------------------------------------------+
                                         | HTTP/HTTPS
+-----------------------------------------------------------------------------------+
|                                  接入层 (Access Layer)                            |
|                 [ 云厂商 SLB / WAF ]  =>  [ Nginx 集群 (动静分离) ]               |
+-----------------------------------------------------------------------------------+
                                         | RESTful API
+-----------------------------------------------------------------------------------+
|                                 网关层 (API Gateway)                              |
|                          [ Spring Cloud Gateway / ShenYu ]                        |
...
|  |   用户服务     |  |   权限服务     |  |   订单服务     |  |   消息服务     |   |
|  +----------------+  +----------------+  +----------------+  +----------------+   |
|  |   基础配置     |  |   报表服务     |  |   工作流服务   |  |   支付服务     |   |
|  +----------------+  +----------------+  +----------------+  +----------------+   |
+-----------------------------------------------------------------------------------+
                                         |
+-----------------------------------------------------------------------------------+
|                             中间件与支撑层 (Middleware Layer)                     |
|  +----------------+  +----------------+  +----------------+  +----------------+   |
|  |   Redis缓存    |  |   Kafka消息队列|  | XXL-JOB调度    |  | Elasticsearch  |   |
|  +----------------+  +----------------+  +----------------+  +----------------+   |
+-----------------------------------------------------------------------------------+
                                         |
+-----------------------------------------------------------------------------------+
|                                数据存储层 (Data Layer)                            |
|  +--------------------------------+   +---------------------------------------+   |
|  | MySQL 8.0+ 集群 (主从/读写分离)|   | MinIO / 阿里云OSS (非结构化文件存储)  |   |
|  +--------------------------------+   +---------------------------------------+   |
+-----------------------------------------------------------------------------------+
```
